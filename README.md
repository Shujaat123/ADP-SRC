...
The public repository intentionally contains **code, data, trained models, and numerical results only**. Manuscript sources, LaTeX files, bibliography files, and manuscript-generation utilities are not distributed here.
...
scripts/          Experiment, verification, model-fitting, and inference scripts
...
The audit checks the dataset SHA-256 hash, expected repeated-CV outputs, model-selection record, headline metrics, common-width comparison, CD-HIT50 results, dictionary dimensions, saved model, and absence of private manuscript artifacts.
...
The serialized model is fitted on all 472 benchmark peptides for future inference. Reported performance comes exclusively from held-out outer-fold predictions, not from this all-data model.
...
If you use this repository, cite the corresponding publication and archived software release. Citation metadata for the software are provided in `CITATION.cff`.
